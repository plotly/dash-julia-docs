...
An `Element` component. `Element` is a wrapper for the `<element>` HTML5 element.

OBSOLETE: `<element>` is included for completeness, but should be avoided as it is not supported by any modern browsers.

For detailed attribute info see: [https://developer.mozilla.org/en-US/docs/Web/HTML/Element/element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/element)
#### Keyword arguments:
