@doc_chapter "/dash_data_table/interactivity" begin
    ...
    @layout html_div() do
        html_h1("DataTable Interactivity"),
        dcc_markdown("""
        ...
        html_blockquote([
            dcc_markdown("""
                Note: simple strings can be entered plain, but if you have spaces or 
                special characters (including -, particularly in dates) you need to 
                wrap them in quotes. Single quotes `'``, double quotes `"``, or backticks ``` all work. [Full filter syntax reference](https://dash-julia.plotly.com/dash_data_table/filtering)
            """)
        ]),
        dcc_markdown("""
            By default, these transformations are done clientside. Your Dash 
            callbacks can respond to these modifications by listening to the 
            `data` property as an `Input`.

            Note that if `data` is an `Input` then the entire data will be 
            passed over the network: if your dataframe is large, then this 
            will become slow. For large dataframes, you can perform the 
            [sorting or filtering in Julia instead](https://dash-julia.plotly.com/dash_data_table/julia_filtering).
        """),
        source"default",
        layout"default",
        html_h3("Row IDs"),
