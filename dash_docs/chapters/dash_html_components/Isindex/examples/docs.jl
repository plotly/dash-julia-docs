...
An `Isindex` component. `Isindex` is a wrapper for the `<isindex>` HTML5 element.

OBSOLETE: `<isindex>` is included for completeness, but should be avoided as it is not supported by any modern browsers.

For detailed attribute info see: [https://developer.mozilla.org/en-US/docs/Web/HTML/Element/isindex](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/isindex)
#### Keyword arguments:
